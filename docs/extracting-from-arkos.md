...
|------|---------|-----------------|
| `idbloader.img` | DDR init + initial program loader | `handhelds/r36h/boot/` (gitignored) |
| `uboot.img` | U-Boot bootloader | `handhelds/r36h/boot/` (gitignored) |
| `trust.img` | ARM Trusted Firmware | `handhelds/r36h/boot/` (gitignored) |
| `gameconsole-r36s.dtb` | U-Boot display DTB | `handhelds/r36h/dtb/` (committed) |
...
cp /mnt/gameconsole-r36s.dtb handhelds/r36h/dtb/
sudo umount /mnt
```
...
The included `handhelds/r36h/dtb/rk3326-gameconsole-r36s-rocknix.dtb` was generated from an ArkOS-R3XS installation and works with Panel 4 (V5) variant R36H units.
